...
import { accentStyles, getCategory, getTool, getToolsByCategory, tools } from "@/lib/tools";
...
export function generateStaticParams() {
  return tools.filter((tool) => tool.kind === "file").map((tool) => ({ slug: tool.slug }));
}
...
export default async function ToolPage({ params }: ToolPageProps) {
  const { slug } = await params;
  const tool = getTool(slug);
  if (!tool || tool.kind !== "file") notFound();
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              {category ? (
                <Link
                  href={category.path}
                  className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 text-sm font-medium text-violet-100 transition hover:bg-white/[0.09]"
                >
              ...
              <p className="mt-5 text-pretty text-lg leading-8 text-slate-300">{tool.description}</p>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between gap-6">
            ...
            {category ? (
              <Link
                href={category.path}
                className="hidden text-sm font-semibold text-violet-200 transition hover:text-white sm:block"
              >
                View all
              </Link>
            ) : null}
